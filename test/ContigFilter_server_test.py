...
class ContigFilterTest(unittest.TestCase):
    ...
    def test_run_ContigFilter_ok(self):
        ...
        ret = self.serviceImpl.run_ContigFilter(self.ctx,
    ...
    def test_run_ContigFilter_min_len_negative(self):
        with self.assertRaisesRegex(ValueError, 'min_length parameter cannot be negative'):
            self.serviceImpl.run_ContigFilter(self.ctx,
                                              {'workspace_name': self.wsName,
                                               'assembly_input_ref': '1/fake/3',
                                               'min_length': '-10'})

    def test_run_ContigFilter_min_len_parse(self):
    ...
    def test_invalid_params(self):
        ...
        # Missing min length
        with self.assertRaises(ValueError):
            impl.run_ContigFilter_max(ctx, {'workspace_name': ws, 'assembly_input_ref': 'x',
                'max_length': 1000000})
        ...
        with self.assertRaises(ValueError):
            impl.run_ContigFilter_max(ctx, {'workspace_name': ws, 'assembly_input_ref': 1,
                'min_length': 1, 'max_length': 1000000})
        # Missing max length
        with self.assertRaises(ValueError):
            impl.run_ContigFilter_max(ctx, {'workspace_name': ws, 'assembly_input_ref': 'x',
